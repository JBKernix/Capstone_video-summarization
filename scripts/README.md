...
| `vlm_upload.py` | VLM 업로드 제한, JPG 검증, OCR-프레임 매칭 |
| `download_models.py` | Hugging Face 모델을 `models/hf_cache/`에 다운로드 |
| `start_server.bat` | Windows Conda 환경에서 Uvicorn 실행 |
...
`vlm_upload.py`의 기본 제한은 다음과 같습니다.
